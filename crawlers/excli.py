...
links = [
    "https://www.excli.de/index.php/excli/issue/archive",
]
...
def parse_paper_info(paper, journal_title, issn, published_year, subject):
    ...
    pdf_url = paper.select_one('a.obj_galley_link.pdf')['href']
    if not pdf_url.startswith('http'):
        pdf_url = domain + '/' + pdf_url
  
    article_url = ''
    title = paper.select_one('h3.title').text.strip()
    authors = ""
...
def parse_archive(base_url, journal_title, eissn, subject):
    ...
    volumes = res.select("ul.issues_archive li a")
