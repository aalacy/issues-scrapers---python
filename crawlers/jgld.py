...
file_paths = []
failed_files = []
PUBLISHER_NAME = "MEDICAL UNIV PRESS"
...
links = [
    "https://www.jgld.ro/jgld/index.php/jgld/issue/archive"
]
...
def filter_year(paper):
    year = paper.h3.text.strip().split()[-1].strip()
    if not year:
        return False
    try:
        year = int(year)
...
def parse_paper_info(paper, journal_title, issn, published_year, subject, domain):
    ...
    authors = ""
    if article.select_one('ol.authors'):
        authors = article.select_one('ol.authors').text.replace('\n\n', '').strip()
    abstract = ''
    if article.select_one('div.abstract'):
        abstract = article.select_one('div.abstract').text.replace('\n\n', '').strip()
    published_at = article.select_one('p.issue').text.split(':')[-1].strip()
...
def parse_issue(issue, journal_title, eissn, year, subject, domain):
    issue_url = issue.h3.a['href']
    if not issue_url.startswith('http'):
        issue_url = domain + issue_url
    papers = bs(fetch_url(issue_url).text, 'lxml').select("div.one-article-intoc")
