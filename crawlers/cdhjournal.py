...
domain = "https://www.cdhjournal.org"
...
file_paths = []
failed_files = []
PUBLISHER_NAME = "EXCLI JOURNAL MANAGING OFFICE"
...
def filter_year(paper):
    year = paper.text.strip().split()[-1].strip()
    if not year:
        return False
    if 's' in year:
        return False
...
def parse_paper_info(paper, journal_title, issn, published_year, subject):
    if not paper.select_one('td.action a'):
        return

    pdf_url = paper.select_one('td.action a')['href']
    if not pdf_url.startswith('http'):
        pdf_url = domain + '/' + pdf_url
  
    article_url = paper.select_one('td.title a')['href']
    title = paper.select_one('td.title').text.strip()
    authors = ""
...
def parse_archive(base_url, journal_title, eissn, subject):
    ...
    volumes = res.select("ol.issues-list li a.issue-date")
    for volume in volumes:
        ...
        papers = volume_data.select('table.browse-issue-table tbody tr')
