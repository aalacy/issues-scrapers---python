...
domains = [
    ...
    "https://www.reabic.net/journals/mbi/"
]
...
links = [
    "https://www.reabic.net/journals/bir/Archive.aspx",
    "https://www.reabic.net/journals/mbi/Archive.aspx"
]
...
def filter_year(paper):
    year = paper.find_previous_sibling('h2').text.strip()
    if not year:
        return False
    try:
        year = int(year)
...
def parse_paper_info(paper, journal_title, issn, published_year, subject, domain):
    ...
    pdf_url = domain + str(published_year) + paper.find('a', href=re.compile(r".pdf$"))['href'].replace('./', '/')
    authors = ""
    abstract = ''
...
def parse_issue(issue, journal_title, eissn, year, subject, domain):
    ...
    papers = bs(fetch_url(issue_url, need_proxies=True).text, 'lxml').select("div.tbl > table > tbody > tr > td > table > tbody > tr")
